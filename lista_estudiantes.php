...
// Consulta SQL para obtener todos los estudiantes activos. 
// DataTables se encargará del filtrado y ordenamiento en el lado del cliente.
$sql = "SELECT id, user, email, fecha_registro FROM usuarios WHERE rol = 'estudiante' AND estado = 'activo'";
...
<body>
                ...
                <?php if ($result && $result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        ...
                        <tr data-id="<?= htmlspecialchars($row['id']) ?>">
                            ...
                            <!-- EDITADO: Se formatea la fecha a dd-mm-yyyy -->
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#estudiantesTable').on('click', '.btn-delete-user', function() {
                    ...
                    if (result.isConfirmed) {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                $('#estudiantesTable').DataTable().row(row).remove().draw();
